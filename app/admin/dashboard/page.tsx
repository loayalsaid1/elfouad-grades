...
import { Upload, Settings, Users, Database, TestTube, History, FileDiff } from "lucide-react"
    ...
    <div className="h-full p-6">
      ...
      <header className="mb-8">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
          <div>
            <h1 className="text-3xl font-bold text-[#223152] mb-2">Admin Dashboard</h1>
            <p className="text-gray-600 mb-4">El Fouad Schools Group Management</p>
        ...
        <Card className="hover:shadow-xl transition-all duration-300 hover:scale-[1.02] border-2 hover:border-[#223152]">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div className={`p-2 rounded-full ${stats.systemStatus === "enabled" ? "bg-green-100" : "bg-red-100"}`}>
              <div
                className={`h-4 w-4 rounded-full ${stats.systemStatus === "enabled" ? "bg-green-500" : "bg-red-500"}`}
              />
            </div>
          </CardHeader>
          <CardContent>
          ...
          <CardContent>
            {lastLogin ? (
              <div className="text-xs text-gray-700 space-y-1">
                <div>
                  <span className="font-semibold">Date:</span> {new Date(lastLogin.created_at).toLocaleDateString()}
                </div>
                <div>
                  <span className="font-semibold">IP:</span>{" "}
                  {lastLogin.ip_address || <span className="italic text-gray-400">N/A</span>}
                </div>
              </div>
            ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-8">
        ...
        >
          ...
          <CardContent>
            <Button
              variant="outline"
              className="w-full border-[#223152] text-[#223152] hover:bg-[#223152] hover:text-white transition-all duration-300 bg-transparent"
            >
              Open Settings
            </Button>
          </CardContent>
        ...
        >
          <CardHeader className="flex-grow">
            ...
            <CardDescription>Search and view student results as a normal user would</CardDescription>
          </CardHeader>
          <CardContent>
            <Button
              variant="outline"
